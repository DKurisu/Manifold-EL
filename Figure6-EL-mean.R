...
############################
#first sample
gfA <- array(0,dim=c(MC,n))
gfC <- array(0,dim=c(MC,n,2))
gfD <- array(0,dim=c(MC,n,2))
#second sample
gfA2 <- array(0,dim=c(MC,n))
gfC2 <- array(0,dim=c(MC,n,2))
gfD2 <- array(0,dim=c(MC,n,2))
#################################
...
pb <- progress_bar$new(total = MC,
                       ...
                       clear = TRUE)
kappa1 <- 20 #parameter of concentration of von Mises-Fisher dist. 
