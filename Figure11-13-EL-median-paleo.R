...
brks <- seq(min(Fre_med),max(Fre_med),by=0.03)
fig <- levelplot(z~x*y,data=Fmed_grid, asp=1,col.regions=terrain.colors,xlab="xi",ylab="theta")#,at=brks)
fig + as.layer(contourplot(z~x*y, data=Fmed_grid, asp=1, at=brks, labels=F))
######################
#C-matrix
...
for(j in 1:ph_N){
  ...
  for(k in 1:th_N){
    ...
    EL_dual <- function(lam){
      g_vec2 <- rep(0,n)
      for(i in 1:n){
        g_vec2[i] <- -2*log(1+sum(lam*g_func_med(X[i,],th_vec[k],ph_vec[j])))
      }
      return(sum(g_vec2))
    }
...
th_med <- th_cand[ind_Fmed_th]
ph_med <- ph_cand[ind_Fmed_ph]
brks <- seq(min(EL_grid$z),max(EL_grid$z),by=10)
fig <- levelplot(z~x*y,data=EL_grid, asp=1,col.regions=terrain.colors,xlab=expression(xi),ylab=expression(theta))#,at=brks)
