...
Lam_11_f <- function(x,th,ph){#x: vector, th, ph: angle of the null Frechet median
  m1 <- sin(th)*cos(ph)
  m2 <- sin(th)*sin(ph)
  m3 <- cos(th)
  m <- c(m1,m2,m3)
  ...
  CL1 <- -sin(1)^2*(AL1/(BL1^3))*(gL1^2)
  CL2 <- -(AL1/BL1)*(cos(1)-2*sin(1))
  DL1 <- CL1 + CL2
  return(DL1)
}
Lam_12_f <- function(x,th,ph){#x: vector, th, ph: angle of the null Frechet median
  m1 <- sin(th)*cos(ph)
  m2 <- sin(th)*sin(ph)
  m3 <- cos(th)
  m <- c(m1,m2,m3)
  ...
  DL2 <- -sin(1)^2*(AL2/(BL2^3))*gL2*gL3
  return(DL2)
}
Lam_22_f <- function(x,th,ph){#x: vector, th, ph: angle of the null Frechet median
  m1 <- sin(th)*cos(ph)
  m2 <- sin(th)*sin(ph)
  m3 <- cos(th)
  m <- c(m1,m2,m3)
  ...
  CL3 <- -sin(1)^2*(AL3/(BL3^3))*(gL4^2)
  CL4 <- -(AL3/BL3)*(cos(1)-2*sin(1))
  DL3 <- CL3 + CL4
  return(DL3)
}
